...
export class Migration20250812165018 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table \`order\` (\`id\` integer not null primary key autoincrement, \`total\` numeric(10,2) not null, \`payment_method\` text check (\`payment_method\` in ('pix', 'dinheiro', 'cartao')) not null, \`status\` text check (\`status\` in ('pendente', 'preparando', 'pronto')) not null default 'pendente', \`created_at\` datetime not null);`);
